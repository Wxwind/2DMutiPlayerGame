...
namespace Panel
{
    class RegisterPanel:BasePanel
    {
        ...
        private void SendRequest_Register()
        {
            if (CheckInput())
            {
                var csRegister = new C2S_Register {Username = userName.text, Password = passWord.text,PlayerName =playerName.text};
        ...
        private bool CheckInput()
        {
            ...
            if (userName.text.Length>20||userName.text.Length==0||passWord.text.Length>20||passWord.text.Length==0
                )
            {
